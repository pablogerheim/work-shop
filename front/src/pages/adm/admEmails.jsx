...
import {
  getEmail, deleteEmail, activeEmail, getLastId,
} from '../../data/admData';
...
function AdmEmails() {
  ...
  function hendleSerach(wanted) {
    ...
    if (emailData) {
      const filtredEmails = emailData.filter((obj) => obj.name.toLowerCase().includes(wanted.toLowerCase()));
      setEmailData(filtredEmails);
    }
  }

  function email(selectedEmail, {
    emailId, name, active, email,
  }) {
      ...
      <section className="screen">
        <div className="p-4">
          ...
          {lastId > 0 ? (
            <div className="mt-1">
              <p>
                Total de emails já cadastrados
                {lastId}
              </p>
            </div>
          ) : <p className="mt-1"> Sem informações</p>}
