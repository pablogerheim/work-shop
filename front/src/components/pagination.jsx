...
function Pagination({
  ...
  useEffect(() => {
    if (dataLength > 0) {
      const calcPage = (dataLength / 12).toFixed(1);
  ...
  function activeStyle(numberpage) {
    const styleA = numberpage === page ? 'border px-2 mb-2 rounded bg-blue-400' : 'border px-2 mb-2 rounded ';
    return styleA;
  }
  ...
  function typeOfPagination() {
    if (dataLength <= 12) {
      return (
        <div>
          {' '}
          <button className="border px-2 mb-2 rounded bg-blue-400">1</button>
        </div>
      );
    }
    if (numberPages.length <= 8) {
      return (
        <>
          {numberPages.map((numeroPagina) => (<button type="button" onClick={(evt) => controlPage(numeroPagina)} key={v4()} className={activeStyle(numeroPagina)}>{numeroPagina}</button>))}
        </>
      );
    } if (page <= 3 || page >= numberPages.length - 2) {
      return (
        <>
  ...
  return (
    <div className="flex justify-around">
      ...
      {typeOfPagination()}
      <button
        disabled={page === numberPages.length}
        onClick={(evt) => controlPage(page + 1)}
      >
