    ...
    try {
        ...
        if (
            ...
            return res.status(422).json({
                msg: 'O Nome, Image, Descrição, url, ativo e autoexplan são obrigatórios!',
            });
        }
...
async function updateProduct(req, res, next) {
    try {
        const { name, image, description, url, active, autoexplan, productId } =
            req.body;
        if (
            !name == null
            || image == null
            ...
            return res.status(422).json({
                msg: 'O Nome, Image, Descrição, url, ativo, id e autoexplan são obrigatórios!',
            });
        }
...
async function updateContact(req, res, next) {
    try {
        ...
        if (!name == null || email == null || telephone == null) {
            return res
                .status(422)
                .json({ msg: 'O nome, email e telefone são obrigatórios!' });
        }
